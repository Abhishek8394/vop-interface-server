...
// ---------------------------------- Talkback connection --------------------------------------------------------------------------------------------
/**
* Connection object that is used to transfer messages to users.
* @param {Socket} socket - connection over which to talk to user.
*/
var TalkbackSocket = function(socket){
	// maybe add user ID
	this.socket = socket;
	this.readHandler = undefined;
	this.writeHandler = undefined;
...
/**
 * Initialize the connection, setup handlers
 */
TalkbackSocket.prototype.initializeConnection = function(){
	selfPtr = this;
	process.nextTick(function(){				
		selfPtr.socket.on('message',function(b){selfPtr.bufferedReader(b);});
	});
 ...
 * @param {String} buffer - Data received on web socket.
 */
TalkbackSocket.prototype.bufferedReader = function(buffer){
	this.msgBuffer+=buffer;
	// console.log(this.msgBuffer);		
	var res = utility.extractAllJSONStrings(this.msgBuffer);
	this.msgBuffer = res.leftover;
	for(message of res.jsonStringArray){	
		...
		// forward all messages to Connection Manager.
		cmEventEmitter.emit("talkbackReceivedData",this,incMsg); 
		console.log("----------------");
...
/**
 * Write to the socket.
 * @param {object} msgObj - Data to send to socket.
 */
TalkbackSocket.prototype.write = function(msgObj){
	msgObj.currentTime = new Date();
	this.writeHandler(this.socket,msgObj);
	console.log("to user");
	console.log(JSON.stringify(msgObj,null," "));
...
* @constructor
* @TODO
* - Fix talkback socket thing
* - Migrate VOP to listen from some other fields that "response". This escaped my todo list before.
*/
function ConnectionsManager(){
	this.appConnections={};
	// currently handles just one talkback socket. Handle multiple and do routing
	...
	cmEventEmitter.on("talkbackReceivedData", function(conn, msgObj){
		selfPtr.talkbackMessageReceiver(conn, msgObj);
	});
	cmEventEmitter.on("sendToTalkback", function(conn, msgObj){
		selfPtr.sendToTalkback(conn, msgObj);
	});
};
...
ConnectionsManager.prototype.createTalkbackConnectionObject=function(socket){
	this.talkbackSocket = new TalkbackSocket(socket);
	this.talkbackSocket.initializeConnection();
}
...
/**
* Handle Incoming data from Apps. If it contains "response", route that to client too.
...
ConnectionsManager.prototype.appMessageReceiver=function(appconn,msgObj){
	// forward to VOP
	if("response" in msgObj){
		// console.log(msgObj["response"]);
		cmEventEmitter.emit("sendToTalkback", appconn.getName(), msgObj["response"]);
	}
...
/**
* Received a message from talkback, handle it.
* @param {Connection} conn - The connection over which msg was received
* @param {object} msgObj - The msg object received from socket.
*/
ConnectionsManager.prototype.talkbackMessageReceiver=function(conn, msgObj){
	console.log("got from talkback, shouldn't have", msgObj);
};
/**
* Received a message for talkback, send it to talkback.
* Also wraps it under {from: senderName, msg: msgObj}
* @param {string} senderName - name of app that sent it.
* @param {object} msgObj - The msg object received from socket.
*/
ConnectionsManager.prototype.sendToTalkback=function(senderName, msgObj){
	if(this.talkbackSocket){
		this.talkbackSocket.write({from: senderName, msg: msgObj});
	}
};

/**
* Artificially emit an event for the connection manager. Use for debugging only.
* @param {string} eventName - The event to trigger
* @param {string} senderName - emulate sender name.
* @param {object} msgObj - The msg object received from socket.
*/
ConnectionsManager.prototype.emitEvent = function(eventName, senderName, msgObj) {
	cmEventEmitter.emit(eventName, senderName, msgObj);
};
